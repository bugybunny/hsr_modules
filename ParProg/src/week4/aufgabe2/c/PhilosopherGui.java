...
class PhilosopherPanel extends JPanel implements Observer {
	...
	public void paint(Graphics g) {
		...
		for (int i = 0; i < philCount; i++) {
			...
			g.fillArc((int) Math.round(transCenterX + tableRadius
					* Math.cos(i * phi)), (int) Math.round(transCenterY
					+ tableRadius * Math.sin(i * phi)), 2 * plateRadius,
					2 * plateRadius, 0, 360);
			...
			g.drawLine((int) Math.round(centerX
					+ (tableRadius - halfStickLength)
					* Math.cos(i * phi + phi / 2 + tetaIn)), (int) Math
					.round(centerY + (tableRadius - halfStickLength)
							* Math.sin(i * phi + phi / 2 + tetaIn)), (int) Math
					.round(centerX + (tableRadius + halfStickLength)
							* Math.cos(i * phi + phi / 2 + teta)), (int) Math
					.round(centerY + (tableRadius + halfStickLength)
